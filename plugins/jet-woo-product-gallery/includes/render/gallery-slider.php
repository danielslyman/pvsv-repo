...
if ( ! class_exists( 'Jet_Gallery_Slider' ) ) {
	...
	class Jet_Gallery_Slider extends Jet_Gallery_Render_Base {
		...
		public function get_slider_data_settings() {
			...
			$options = [
				'autoHeight'               => 'horizontal' === $slider_settings['pagination_direction'] ? $slider_settings['slider_equal_slides_height'] : true,
			...
			return sprintf(
				'data-swiper-settings="%s" data-swiper-thumb-settings="%s"',
				htmlspecialchars( json_encode( $options ) ),
				htmlspecialchars( json_encode( $thumb_options ) )
			);
