...
let elementSettings=dceGetElementSettings($scope);let container=$scope.find('.dce-posts-container.dce-skin-grid .dce-posts-wrapper');let layoutMode=elementSettings[dceDynamicPostsSkinPrefix+'grid_type'];let $filterItems=$scope.find('.dce-filters .filters-item');let defaultFilter=$filterItems.first().find('a').attr('data-filter');container.imagesLoaded(()=>{container.isotope({itemSelector:'.dce-post-item',layoutMode:'masonry'===layoutMode?'masonry':'fitRows',sortBy:'original-order',filter:defaultFilter,percentPosition:!0,masonry:{columnWidth:'.dce-post-item'}})});$filterItems.on('click','a',function(e){$(this).parent().siblings().removeClass('filter-active');$(this).parent().addClass('filter-active');let filterValue=$(this).attr('data-filter');container.isotope({filter:filterValue,});if(elementSettings.grid_filters_match_height){container.on('layoutComplete',function(event,laidOutItems){jQuery.fn.matchHeight._update()})}
